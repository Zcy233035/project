...
public class TERenderer {
    ...
    public void renderFrame(TETile[][] world, ArrayList<Point> coins, ArrayList<Rectangle> rooms, boolean turnoffsign) {
        StdDraw.clear(new Color(0, 0, 0));
        drawTiles(world);
        drawTheLight(coins, rooms, world);
        if (turnoffsign) {
            turnOffTheLight(coins, rooms, world);
        }
        StdDraw.show();
    }
    ...
    public void drawTheLight(ArrayList<Point> coins, ArrayList<Rectangle> rooms, TETile[][] worldGrid) {
            ...
            for (Rectangle room : rooms) {
                for (int x = room.x; x < room.x + room.width; x++) {
                    for (int y = room.y; y < room.y + room.height; y++) {
                        if (worldGrid[x][y] != Tileset.WALL && worldGrid[x][y] != Tileset.TREE) { // Check for walls and coins
                            int distance = Math.abs(x - coinX) + Math.abs(y - coinY);
                            if (distance == 1) {
                                StdDraw.setPenColor(new Color(0, 150, 255, 185));
                                StdDraw.filledRectangle(x + 0.5, y + 0.5, 0.5, 0.5);
                            } else if (distance == 2) {
                                StdDraw.setPenColor(new Color(0, 70, 255, 150));
                                StdDraw.filledRectangle(x + 0.5, y + 0.5, 0.5, 0.5);
                            } else if (distance == 3) {
                                StdDraw.setPenColor(new Color(0, 50, 255, 135));
                                StdDraw.filledRectangle(x + 0.5, y + 0.5, 0.5, 0.5);
                            } else if (distance == 4) {
                                StdDraw.setPenColor(new Color(0, 30, 255, 120));
                                StdDraw.filledRectangle(x + 0.5, y + 0.5, 0.5, 0.5);
                            } else if (distance == 5) {
                                StdDraw.setPenColor(new Color(0, 30, 255, 120));
                                StdDraw.filledRectangle(x + 0.5, y + 0.5, 0.5, 0.5);
    ...
    public void turnOffTheLight(ArrayList<Point> coins, ArrayList<Rectangle> rooms, TETile[][] worldGrid) {
        Point firstCoin = coins.get(0);
        int cx = firstCoin.x;
        int cy = firstCoin.y;
        ...
        for (int x = firstRoom.x; x < firstRoom.x + firstRoom.width; x++) {
            for (int y = firstRoom.y; y < firstRoom.y + firstRoom.height; y++) {
                worldGrid[x][y].draw(x + xOffset, y + yOffset);
            }
        }
    }
